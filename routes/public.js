...
router.get('/', async (req, res) => {
  try {
    const news = await getAll('SELECT * FROM news WHERE is_published = 1 ORDER BY published_at DESC LIMIT 6');
    const galleryPhotos = await getAll(`SELECT id, title, image FROM news WHERE is_published = 1 AND image IS NOT NULL AND image != '' ORDER BY published_at DESC LIMIT 6`);
    res.render('index', { title: 'Главная', news, galleryPhotos, activePage: 'home' });
  } catch (err) {
    console.error(err);
    res.status(500).render('404', { title: 'Ошибка' });
  }
});
...
router.get('/news', async (req, res) => {
  try {
    ...
    if (category) {
      news = await getAll(
        'SELECT * FROM news WHERE is_published = 1 AND category = $1 ORDER BY published_at DESC LIMIT $2 OFFSET $3',
        [category, limit, offset]
      );
      countRow = await getOne('SELECT COUNT(*)::int as cnt FROM news WHERE is_published = 1 AND category = $1', [category]);
    } else {
      news = await getAll(
        'SELECT * FROM news WHERE is_published = 1 ORDER BY published_at DESC LIMIT $1 OFFSET $2',
        [limit, offset]
      );
      countRow = await getOne('SELECT COUNT(*)::int as cnt FROM news WHERE is_published = 1');
    }

    const total = countRow.cnt;
    const totalPages = Math.ceil(total / limit);
    const categories = await getAll('SELECT DISTINCT category FROM news WHERE is_published = 1');
...
router.get('/news/:id', async (req, res) => {
  try {
    const item = await getOne('SELECT * FROM news WHERE id = $1 AND is_published = 1', [req.params.id]);
    if (!item) return res.status(404).render('404', { title: 'Не найдено' });

    const related = await getAll(
      'SELECT * FROM news WHERE is_published = 1 AND id != $1 AND category = $2 ORDER BY published_at DESC LIMIT 3',
...
router.get('/events', async (req, res) => {
  try {
    const upcoming = await getAll('SELECT * FROM events WHERE is_past = 0 ORDER BY event_date ASC');
    const past = await getAll('SELECT * FROM events WHERE is_past = 1 ORDER BY event_date DESC');
...
router.get('/documents', async (req, res) => {
  try {
    ...
    const categories = await getAll('SELECT DISTINCT category FROM documents');
